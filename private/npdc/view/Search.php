...
class Search extends Base{
    ...
    public function getForm() {
        ...
        $this->types = get_object_vars(
            $this->controller->form->fields->type->options
        );
        if (
            count(
                get_object_vars($this->controller->form->fields->type->options)
            ) < 2
        ) {
        ...
        if (empty($this->search)) {
            ...
            if (\Lootils\Uuid\Uuid::isValid($this->search)) {
                ...
                if (in_array('organization', $this->type)) {
                    ...
                    if (count($orgs) > 0) {
                        $this->mid .= count($orgs) . ' Organization'
                            . (
                                count($orgs) > 1
                                ? 's'
                                : ''
                        ...
                        foreach ($orgs as $org) {
                            $this->searchFields['organization'][] =
                                $org['organization_id'];
                            $this->mid .= '<li><a href=' . BASE_URL 
                                . '/organization/' . $org['organization_id']
                                . '>' . $org['organization_name'] . '</a></li>';
                        }
                        $this->mid .= '</ul>';
                    } else {
                ...
                if (count($this->searchFields) > 0) {
                    ...
                    foreach (
                        ...
                        if (
                            ...
                            if (
                                count($this->searchFields['organization']) > 0
                            ) {
                                foreach (
                                    $model->getList(
                                        ['organization' => $this->searchFields['organization']]
                                    )
                                    as $row
                                ) {
                        ...
                        foreach (
                            $pubModel->searchByFreeOrganization($this->search)
                            as $row
                        ) {
                        ...
                        if (array_key_exists('person', $this->searchFields)) {
                            ...
                            foreach (
                                $pubModel->searchByFreePerson($this->search)
                                as $row
                            ) {
                                $row['content_type'] = 'Publication';
                                $key = 'publication' . $row['publication_id'];
