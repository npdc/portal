...
$citations = $this->model->getCitations(
    $this->data['dataset_id'],
    $this->data['dataset_version'],
    'other'
);
if (count($citations) > 0) {
    ...
    foreach ($citations as $citation) {
        echo '<p>' . $citation['creator'] . ' ('
            . substr($citation['release_date'], 0, 4) . '). <em>'
...
$related = $this->model->getRelatedDatasets(
    $this->data['dataset_id'],
    $this->data['dataset_version']
);
if (count($related) > 0) {
    ...
    foreach ($related as $set) {
        if (!empty($set['doi'])) {
            $l = '<a href="https://dx.doi.org/' . $set['doi']
                . '">https://dx.doi.org/' . $set['doi'] . '</a>';
        ...
        echo '<li>' . $l . ' - <em>'
            ...
            . (
                empty($set['relation'])
                ? ''
                : '<span style="font-size:80%"><br/>Relation: '
                    . $set['relation'] . '</a>'
            )
            . '</em></li>';
    }
