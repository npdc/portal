...
foreach($this->versions as $version){
	if(in_array($version['record_status'], ['published', 'archived'])){
		$versions[] = '<a href="'.BASE_URL.'/'.$this->controller->name.'/'.$version['uuid'].'">'.$version[$this->controller->name.'_version'].' ('.($versin['record_status'] === 'published' ? 'current' : $version['record_status']).')</a>';
	}
}
if(count($versions) === 1){
