	...
	public function showItem(){
		if($this->controller->file === false){
			...
			switch($this->controller->status){
				case 'ready':
					$url = BASE_URL.'/Download/'.$this->controller->file['filename'].'.zip';
				...
				default:
					...
					$this->mid = 'Generating download, please keep this page open, it will autorefresh every '.$refresh.' seconds<meta http-equiv="refresh" content="'.$refresh.'"><br/><br/>'.nl2br(file_get_contents(\npdc\config::$downloadDir.'/'.$this->controller->file['filename'].'.log'));
			}
		}
	}
}