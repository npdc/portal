...
if(!empty($this->data['zip_id'])){
	...
	echo '<p><a href="'.BASE_URL.'/download/'.$zip['filename'].'">Download</a></p>';
}
if($this->modelDataset->isEditor($this->data['dataset-id'], $this->session->userId) || $this->session->userLevel === NPDC_ADMIN){
	...
	if($this->data['permitted'] === null){
		...
		<h4>Allow*:</h4>
		<div class="cols">
			<div><input type="radio" name="allow" id="allow_yes" value="yes" <?=$_POST['allow'] === 'yes' ? 'checked' : ''?> /><label for="allow_yes">Yes</label></div>
			<div><input type="radio" name="allow" id="allow_no" value="no" <?=$_POST['allow'] === 'no' ? 'checked' : ''?> /><label for="allow_no">No</label></div>
		</div>
		<h4>Comments:</h4>
