...
class Vocab {
	//parts between { and } are only used when displaying a row, not for list, so for lists these parts are stripped in getList, for single the brackets are removed in formatTerm
	private $fields = [
		'vocab_chronounit'=>['eon', 'era', 'period', 'epoch', 'stage'],
		'vocab_instrument'=>['category', 'class', 'type', 'subtype', ['short_name', 'long_name']],
		'vocab_location'=>['location_category', 'location_type', 'location_subregion1', 'location_subregion2', 'location_subregion3'],
		'vocab_platform'=>['category', 'series_entity', ['short_name',' long_name']],
		'vocab_res_hor'=>['{hor_}range'],
		'vocab_res_vert'=>['{vert_}range'],
		'vocab_res_time'=>['{time_}range'],
	...
	public function getList($vocab, $filter = null){
		...
		foreach($data as $row){
			...
			if(empty($filter) || strpos(strtolower($term), strtolower($filter)) !== false){
				if(strlen($term) > 0){
	...
	public function formatTerm($vocab, $row, $last = true, $shorten=false){
		...
		for($i=0;$i<($last ? count($fields) : count($fields)-1);$i++){
			//if field has both short and long term the short should be between brackets, this tests this
			if(is_array($fields[$i])){
				...
				if(!empty($row[$fields[$i][0]])){
					//test if long term has value, if not, display short without brackets
					if(empty($row[$fields[$i][1]])){
						$part .= $row[$fields[$i][0]];
					} else {
						$part .= ' ('.$row[$fields[$i][0]].')';
