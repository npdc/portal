...
$().ready(function(){
    ...
    $('select[name="license_id"]').on('change',function(){
        url = baseUrl + '/license/' + this.value + '.json';
        $.ajax(url).done(function(data){
            target = $('textarea[name="access_constraints"]');
            curval = target.val();
            if(curval.length == 0){
                target.val(data.access_constraints);
            }
            usetarget = $('textarea[name="use_constraints"]');
            curval = usetarget.val();
            if(curval.length == 0){
                usetarget.val(data.use_constraints);
            }
        });
    });
    //suggestions
