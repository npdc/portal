...
if (count($datasets) === 0) {
    ...
    foreach ($datasets as $dataset) {
        ...
        . '</h4><div class="hiddenSubDiv"><p>'
        . $dataset['date_start'] . ' - ' . $dataset['date_end'] . ' |&nbsp;'
        . $dataset['dif_id'];
        $people = $datasetModel->getPersons(
            $dataset['dataset_id'],
        ...
        $projects = $datasetModel->getProjects(
            $dataset['dataset_id'],
            $dataset['dataset_version'],
            false
        );
        if (count($projects) > 0) {
            ...
            foreach ($projects as $project) {
                echo '<p><a href="' . BASE_URL . '/project/'
                    . $project['project_id'] . '">' . $project['title'] . '</a>'
                . (
        ...
        $publications = $datasetModel->getpublications(
            $dataset['dataset_id'],
            $dataset['dataset_version'],
            false
        );
