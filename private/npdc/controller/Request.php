...
class Request extends Base {
    ...
    public function __construct($session) {
        ...
        if (
            ...
            && array_key_exists('reason', $_POST)
        ) {
            if (empty($_POST['allow'])) {
                $this->error = 'Please select if the user can get access to the'
                    . ' requested file(s)';
            } elseif ($_POST['allow'] === 'no' && empty($_POST['reason'])) {
                $this->error = 'Please provide a reason the user cant get '
                    . 'access to the requested file(s)';
            } else {
                $this->saveAccess();
            }
        }
    ...
    private function saveAccess() {
        $this->model->updateRequest(
            \npdc\lib\Args::get('id'),
            [
                'permitted' => $_POST['allow']==='yes' ? 1 : 0,
                'response' => $_POST['reason'],
                'response_timestamp' => date('Y-m-d h:i:s'),
                'responder_id' => $this->session->userId
            ]
        );
        $request = $this->model->getById(\npdc\lib\Args::get('id'));
        $personModel = new \npdc\model\Person();
        ...
        if ($_POST['allow']==='yes') {
            ...
            $zip->create(
                preg_replace(
                    '/[^A-Za-z0-9\-_]/', //^ = any char not in this range
                    '',
                    str_replace(' ', '_', $person['name'])
                )
            );
        ...
        $mailText = 'Hi ' . $person['name'] . "\r\n\r\nYour request to access '
            . 'the files below has been " 
            . ($_POST['allow'] === 'yes' ? 'accepted' : 'rejected')
            . '. ';
        ...
        $mailText .= "\r\n";
        if ($_POST['allow'] === 'yes') {
            $mailText .= 'You can download the files at ' . getProtocol()
                . $_SERVER['HTTP_HOST'] . $zip->redirect . ".zip\r\n\r\n";
        }
        $mailText .= "\r\n\r\nKind regards,\r\n" . \npdc\config::$siteName;
        $mail = new \npdc\lib\Mailer();
