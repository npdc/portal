...
class Form {
	...
	private $adminOverrule = '<div id="adminoverrule"><p><i>As admin you can overrule the checking of a record before it is being saved to the database using the checkbox below. Some fields are required on database level, if you left one of those fields empty your record will not be saved and the site will give a server error (you will not get a message telling what wnet wrong). Please use this function with care!</i></p><input type="checkbox" id="adminoverruleinput" name="adminoverrule" value="adminoverrule"><label for="adminoverruleinput">I am aware of the risks and wish to use the admin override when submitting this page.</label></div>';
	private $formId;
	private $active = 0;
	private $required;
	private $collapsible;
			...
			foreach($field->options as $option_id=>$label){
				$input .= '<div><input name="'.$fieldName.'" '
					...
					.(in_array($option_id, $_SESSION[$this->formId]['data'][$id]) 
						? ' checked' 
						:'')
					.'><label for="'.$fieldName.'_'.$option_id.'"> '.$label.'</label></div>';
			}
			$input .= '</div>';
		}
		return $input;
