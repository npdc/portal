...
class Args {
    ...
    private static function parse() {
        ...
        if (array_key_exists('logintype', self::$args)) {
            ...
            if (array_key_exists('loginid', self::$args)) {
                self::$args['id'] = self::$args['loginid'];
            }
        }
            ...
            if (\Lootils\Uuid\Uuid::isValid(self::$args['uuid'])) {
                ...
                foreach (
                    ...
                    if (method_exists($mName, 'getByUUID')) {
                        ...
                        if (!empty($r)) {
                            self::$args = array_merge(
                                [
                                    'type'=>$cType,
                                    'id'=>$r[$cType.'_id'],
                                    'version'=>$r[$cType.'_version']
                                ],
                                self::$args
                            );
                            break;
