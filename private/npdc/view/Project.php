...
class Project extends Base{
    ...
    public function showList() {
        ...
        for (
            ...
            if (count($parent) > 0) {
                $add .= '<div class="parent">Subproject of \''
                    . $parent[0]['title'] . '\'</div>';
            }
        ...
        $this->mid = '<div class="related"><i>Projects with related projects'
            . '</i> <a href="javascript:toggleRelated(false)">Show related '
            . 'projects</a></div><div class="noRelated"><a href="javascript:'
            . 'toggleRelated(true)">Hide related projects</a></div>'
            . $this->displayTable(
                'project',
                $list2, 
                [
    ...
    public function showItem($project) {
        if (\npdc\lib\Args::get('action') !== 'new') {
            ...
            if (
                ...
                $this->data = $this->model->getById(
                    $project,
                    $this->versions[0]['project_version']
                );
            }
        }
            ...
            if (is_null($this->controller->display)) {
                ...
                if (!$changed) {
                    if ($this->data['record_status'] === 'draft') {
                        $_SESSION['notice'] .= ' Publishing this draft is not '
                            . 'possible since it doesn\'t appear to be '
                            . 'different than the published record.';
                    }
                } elseif (
                    ...
                    if (!$this->canEdit) {
                        ...
                        $_SESSION['notice'] .= ', the current version can be '
                            . 'found <a href="' . BASE_URL . '/project/'
                            . $cur['uuid'] . '">here</a>';
                    }
                }
                $this->title = 'Project - ' . $this->data['title']
                    . (
