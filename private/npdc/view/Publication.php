...
class Publication extends Base{
    ...
    protected $userLevelAdd = NPDC_EDITOR;
    ...
    public function showList() {
        ...
        for (
            ...
            $item = $list[$i];
            $item['publication'] = $this->model->getCitation(
                $item['publication_id'],
    ...
    public function showItem($publication) {
        if (\npdc\lib\Args::get('action') !== 'new') {
            ...
            if (
                ...
                $this->data = $this->model->getById(
                    $publication,
                    $this->versions[0]['publication_version']
                );
            }
        }
        ...
        if ($this->data === false && $publication !== 'new') {
            ...
            if (\npdc\lib\Args::get('action') === 'warnings') {
                ...
                if (!$changed) {
                    ...
                    if (!$this->canEdit) {
                        $cur = $this->model->getById(
                            $this->data['publication_id']
                        );
                        $_SESSION['notice'] .= ', the current version can be '
                            . 'found <a href="' . BASE_URL . '/publication/'
                            . $cur['uuid'] . '">here</a>';
                    }
                }
                $this->title = 'Publication - ' . $this->data['title']
                    . (
                ...
                $this->class = 'detail';
                $this->mid .= parent::parseTemplate(
                    'publication_mid',
                    $model,
                    $this->data
                );
                $this->right = parent::parseTemplate(
                    'publication_right',
                    $model,
                    $this->data
                );
                ...
                $this->title = \npdc\lib\Args::get('action') === 'new'
                    ? 'New publication'
                    : 'Edit publication - ' . $this->data['title'];
    ...
    private function showCitation() {
        $url = $_SERVER['REQUEST_SCHEME'] . '://' . $_SERVER['HTTP_HOST']
            . BASE_URL . '/publication/' . $this->data['uuid'];
        $authors = explode(
            '; ',
            str_replace(
                ' &amp;',
        ...
        $id = str_replace(
            ...
            . substr(
                $citation['title'] ?? $this->data['title'],
                0,
                5
            )
        );
