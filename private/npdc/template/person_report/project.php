...
if (count($projects) === 0) {
    ...
    foreach ($projects as $project) {
        ...
        . (
            $project['record_status'] === 'draft'
            && $project['project_version'] === 1
            ? ' <i>(unpublished)</i>'
            : ''
        )
        . '</h4><div class="hiddenSubDiv"><p>' . $project['date_start'] . ' - '
        . $project['date_end']
        . (
            empty($project['nwo_project_id'])
            ? ''
            : ' |&nbsp;' . $project['nwo_project_id']
        );
        $people = $projectModel->getPersons(
            $project['project_id'],
        ...
        if (count($people) > 1) {
            ...
            foreach ($people as $person) {
                if ($person['person_id'] !== $this->data['person_id']) {
                    echo ($i>0 ? ', ' : ''). '<a href="' . BASE_URL 
                        . '/person/' . $person['person_id'] . '/report">'
                        . $person['name'] . '</a>';
                    $i++;
                }
            }
        }
        echo '<span class="screenonly"> |&nbsp;<a href="' . BASE_URL
            . '/project/' . $project['project_id']
            . '">More details</a></span></p>';
        $datasets = $projectModel->getDatasets(
            $project['project_id'],
            $project['project_version'],
            false
        );
        if (count($datasets) > 0) {
            ...
            foreach ($datasets as $dataset) {
                echo '<p><a href="' . BASE_URL . '/dataset/'
                    . $dataset['dataset_id'] . '">' . $dataset['title'] . '</a>'
                    . (
                        $dataset['record_status'] === 'draft'
                        && $dataset['dataset_version'] === 1
                        ? ' <i>(unpublished)</i>'
                        : ''
                    )
                    . ' |&nbsp;' . $dataset['date_start'] . ' - '
                    . $dataset['date_end'] . ' |&nbsp;' . $dataset['dif_id']
                    . '</p>';
            }
        }
        
        $publications = $projectModel->getpublications(
            $project['project_id'],
            $project['project_version'],
            false
        );
        if (count($publications) > 0) {
            ...
            foreach ($publications as $publication) {
                echo '<p><a href="' . BASE_URL . '/publication/'
                    . $publication['publication_id'] . '">'
                    . $publicationModel->getAuthors(
                        $publication['publication_id'],
                    ...
                    . $publication['title'] . '</a>'
                . (
                    $publication['record_status'] === 'draft'
