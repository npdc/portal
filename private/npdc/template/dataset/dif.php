...
foreach (
    ...
    foreach ([
        ...
        if (array_key_exists($source, $specialFields)) {
            ...
            $data['Dataset_Citation'][$i][$target] = $_SERVER['REQUEST_SCHEME']
                . '://' . $_SERVER['HTTP_HOST'] . BASE_URL . '/dataset/'
                . $this->data['uuid'];
        }
    }
}
...
foreach (
    ...
    foreach ([
        ...
        $data['Personnel'][$i]['Contact_Person'][$target] = 
    ...
    foreach ([
        ...
        $data['Personnel'][$i]['Contact_Person']['Address'][$target] = 
            $organizationDetail[$source];
    }
    foreach (
        [
        ...
        if (
            $personDetails['phone_' . $phoneType . '_public'] === 'yes'
            && !empty($personDetails['phone_' . $phoneType])
        ) {
            $data['Personnel'][$i]['Contact_Person']['Phone'][] = [
                'Number' => str_replace(
                    ['(0)', ' '],
                    '',
                    $personDetails['phone_'.$phoneType]
                ),
                'Type' => $label
            ];
        }
...
foreach (
    ...
    $data['ISO_Topic_Category'][$i] = substr(
        $topic['description'],
        0,
        strpos($topic['description'], ':')
    );
}
    ...
    foreach (
        ...
        foreach (
            ...
            if (!empty($instrument[$source])) {
                $data['Platform'][$i]['Instrument'][$j][$target] = 
                    $instrument[$source];
            }
        }
        foreach (
            ...
            foreach ([
                ...
                if (!empty($sensor[$source])) {
                    $data['Platform'][$i]['Instrument'][$j]['Sensor'][$k][$target] = 
                        $sensor[$source];
                }
            }
        }
...
foreach (
    $this->model->getTemporalCoverages(
        $this->data['dataset_id'],
        $this->data['dataset_version']
    )
    as $i => $tc
            ...
            switch ($group) {
                case 'period':
                    if ($tcg['date_start'] === $tcg['date_end']) {
                        $data['Temporal_Coverage'][$i]['Single_DateTime'][$j] = 
                            $tcg['date_start'];
                    } else {
                        $data['Temporal_Coverage'][$i]['Range_DateTime'][$j] = [
                            'Beginning_Date_Time' => $tcg['date_start'],
                            'Ending_Date_Time' => $tcg['date_end']
                        ];
                    }
                break;
                case 'cycle':
                    foreach (
                        ...
                        $data['Temporal_Coverage'][$i]['Periodic_DateTime'][$j][$target] = 
                            $source === 'sampling_frequency_unit'
                            ? $units[$tcg[$source]]
                            : $tcg[$source];
                    }
                break;
                case 'paleo':
                    $data['Temporal_Coverage'][$i]['Paleo_DateTime'][$j] = [
                        'Paleo_Start_Date' => $tcg['start_value'] . ' '
                            . $tcg['start_unit'],
                        'Paleo_End_Date' => $tcg['end_value'] . ' '
                            . $tcg['end_unit']
                    ];
                    foreach (
                        ...
                        $data['Temporal_Coverage'][$i]['Paleo_DateTime'][$j]['Chronostratigraphic_Unit'][$k] =
                            $this->vocab->formatTerm('vocab_chronounit', $tp);
                    }
                break;
                case 'ancillary':
                    $data['Temporal_Coverage'][$i]['Temporal_Info']['Ancillary_Temporal_Keyword'][$j] = 
                        $tcg['keyword'];
                break;
            }
        }
...
foreach (
    $this->model->getSpatialCoverages(
        $this->data['dataset_id'],
        $this->data['dataset_version']
    )
    as $i => $sc
    ...
    switch ($sc['type']) {
        ...
        case 'Area':
            $latMin = min(
                $points[0]['Point_Latitude'],
                $points[2]['Point_Latitude']
            );
            $latMax = max(
                $points[0]['Point_Latitude'],
                $points[2]['Point_Latitude']
            ...
            $lonMin = min(
                $points[0]['Point_Longitude'],
                $points[2]['Point_Longitude']
            );
            $lonMax = max(
                $points[0]['Point_Longitude'],
                $points[2]['Point_Longitude']
        ...
        case 'LineString':
            $data['Spatial_Coverage'][$i]['Geometry']['Line']['Point'] = 
                $points;
            break;
        case 'Polygon':
            $data['Spatial_Coverage'][$i]['Geometry']['Boundary']['Point'] = 
                $points;
            break;
    }
    foreach (['depth', 'altitude'] as $source) {
        ...
        if (!empty($sc[$source.'_min'])) {
            $data['Spatial_Coverage'][$i]['Minimum_'.ucfirst($source)] =
                $sc[$source.'_min'];
            $showUnit = true;
        }
        if (!empty($sc[$source.'_max'])) {
            $data['Spatial_Coverage'][$i]['Maxnimum_'.ucfirst($source)] =
                $sc[$source.'_max'];
            $showUnit = true;
        }
        if ($showUnit) {
            $data['Spatial_Coverage'][$i][ucfirst($source).'_Unit'] =
                $sc[$source.'_unit'];
        }
    }
}
...
foreach (
    ...
    foreach (
        $this->vocab->getIDNNodes($location['vocab_location_id'])
        as $node
    ) {
        $data['IDN_Node'][$node['vocab_idn_node_id']]['Short_Name'] =
            $node['short_name'];
        if (!empty($node['long_name'])) {
            $data['IDN_Node'][$node['vocab_idn_node_id']]['Long_Name'] =
                $node['long_name'];
        }
    }
}
...
if (!is_null($this->data['originating_center'])) {
    $data['Originating_Center'] = $this->organizationModel->getById(
        $this->data['originating_center']
    )['organization_name'];
}

$datacenters = $this->model->getDataCenter(
    $this->data['dataset_id'],
    $this->data['dataset_version']
);
if (count($datacenters) === 0) {
    foreach (\npdc\config::$dataCenter as $organization_id => $persons) {
        $data['Organization'][$organization_id] = $this->displayDataCenter(
            $organization_id
        );
        foreach ($persons as $person) {
            $data['Organization'][$organization_id]['Personnel'][$person] = [
                'Role' => 'DATA CENTER CONTACT',
                'Contact_Person' => $this->displayDataCenterPersonnel($person)
            ];
        }
    }
} else {
    foreach ($datacenters as $i => $datacenter) {
        $data['Organization'][$i] = $this->displayDataCenter(
            $datacenter['organization_id']
        );
        foreach (
            ...
            $data['Organization'][$i]['Personnel'][$person['person_id']] = [
                'Role' => 'DATA CENTER CONTACT',
                'Contact_Person' => $this->displayDataCenterPersonnel(
                    $person['person_id']
                )
            ];
        }
...
foreach (
    $this->model->getPublications(
        $this->data['dataset_id'],
        $this->data['dataset_version']
    )
    as $i => $publication
) {
    $data['Reference'][$i]['Citation'] = strip_tags(
        $publicationModel->getCitation($publication)
    );
}
...
if (
    ...
    $data['Related_URL'][] = [
        'Title' => 'Download data',
        'Description' => 'Download data from the NPDC',
        'URL' => $_SERVER['REQUEST_SCHEME'] . '://' . $_SERVER['HTTP_HOST']
            . BASE_URL . '/dataset/ ' .$this->data['uuid'] . '/files',
        'URL_Content_Type' => ['Type' => 'GET DATA']
    ];
}
$data['Related_URL'][] = [
    'Title' => 'Data set description at NPDC',
    'Description' => 'Data set description at the NPDC',
    'URL' => $_SERVER['REQUEST_SCHEME'] . '://' . $_SERVER['HTTP_HOST']
        . BASE_URL . '/dataset/' . $this->data['uuid'],
    'URL_Content_Type' => ['Type' => 'DATA SET LANDING PAGE']
];

$nodes = [];
if (in_array($this->data['region'], ['Arctica', 'Bipolar'])) {
    $nodes[] = 'ARCTIC';
    $nodes[] = \npdc\config::$gcmd['node']['arctic'];
}
if (in_array($this->data['region'], ['Antarctica', 'Bipolar'])) {
    $nodes[] = 'AMD';
    $nodes[] = \npdc\config::$gcmd['node']['antarctic'];
}
...
$data['Metadata_Dates'] = [
    'Metadata_Creation' => str_replace(
        ' ',
        'T',
        $this->model->getById($this->data['dataset_id'], 1)['published']
    ),
