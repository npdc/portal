...
class Base {
    ...
    protected function showCanonical() {
        $this->extraHeader = ' <link rel="canonical" href="' . getProtocol()
            . $_SERVER['HTTP_HOST'] . BASE_URL . '/' . $this->controller->name
            . '/' . $this->data['uuid'] . '" />
            ...
            if (\npdc\config::$partEnabled[$type]) {
                ...
                if ($c > 0 && $list) {
                    $_SESSION['notice'] .= 
                        ...
                        . 'You have ' . $c . ' <a href="' . BASE_URL . '/'
                        . $type . '?formid=' . $type . 'list&amp;editorOptions='
                        . 'unpublished">unpublished ' . $type . 's</a>';
                } else {
                    $count += $c;
                }
            }
     ...
     *
     * @param integer $record_id id of entry (of type project, dataset or 
     *                                                              publication)
    ...
    protected function doListStatusChanges($record_id, $record_version) {
        ...
        $statusChanges = $this->model->getStatusChanges(
            $record_id,
            $record_version
        );
        if (count($statusChanges) === 0) {
            $return .= '<span class="noChanges">No status changes recorded for '
                . 'this version</span>';
        } else {
            foreach ($statusChanges as $statusChange) {
                $return .= '<div>'
                    . date('Y-m-d', strtotime($statusChange['datetime']))
                    . ' || ' . $statusChange['name'] . ' || '
                    . $statusChange['old_state'] . ' &rarr; '
                    . $statusChange['new_state'] . '<div class="comment">'
                    . $statusChange['comment'] . '</div></div>';
            }
            if (count($statusChanges) > 1) {
                $return .= '<span onclick="$(\'#statusChanges\').toggleClass('
                    . '\'all\')"><span>Show all ' . count($statusChanges)
                    . ' status changes</span><span>Show only last status change'
                    . '</span></span>';
            }
        }
        $return .= '</div>';
    ...
    protected function makePager($n, $page) {
        $this->pager = '<div class="pager"><span class="hint">Showing results '
            . (
                ($page - 1) * \npdc\config::$rowsPerPage + 1
            )
            . ' to ' . min($page * \npdc\config::$rowsPerPage, $n) . ' of ' 
            . $n . '</span>';
            ...
            $this->pager .= '<span class="numbers">Page:';
                ...
                for ($i = 1; $i <= ceil($n / \npdc\config::$rowsPerPage); $i++) {
                    if (
                        ...
                        if ($i == $page) {
                            $this->pager .= '<span class="page-number active">'
                                . $i . '</span>';
                        } else {
                            $this->pager .= '<span class="page-number"><a href="'
                                . $base_url . $i
                                . (
     ...
     *
     * @param string $class any classnames that need to be added to the table 
     *                              element
     * @param array $data array containing the data for the table
     * @param array $columns column definitions, 
     *                       id(array key in $data element)=>heading 
     *                    or [heading,'array'(indicating need to implode value)]
     * @param array $url parameters for generating url 
     *                       [content_type|'content_type', column id to use]
    ...
    protected function displayTable(
        ...
        if (
            ...
            && in_array('list', explode(' ', $this->class))
        ) {
            $return .= '<div class="add"><a href="' . $url[0] . '/new">Add '
                . $url[0] . '</a>';
        ...
        if (count($data) === 0) {
            ...
            if (
                ...
                $return .= '<div class="screenonly">'
                    . ucfirst($this->controller->name)
                    . 's with a striped background are not yet published but '
                    . 'only visible to editors of that ' . $this->controller->name
                    . ' and administrators.<br/>' . ucfirst($this->controller->name)
                    . 's with an * on the edit button have a draft version</div>'
                    . '<div class="printonly">' . ucfirst($this->controller->name)
                    . 's in <i>italics</i> are not yet published but only '
                    . 'visible to editors of that '.$this->controller->name
            ...
            foreach ($data as $item) {
                if (is_array($url)) {
                    if ($url[0] === 'content_type') {
                        ...
                        $link = BASE_URL . '/' 
                            . strtolower($item['content_type']) . '/' 
                            . $item[$id];
                    } else {
                        $link = empty($item[$url[1]])
                            ? ''
                            : BASE_URL . '/' . $url[0] . '/'
                            . (
                                array_key_exists('uuid', $item)
                ...
                if ($editTable) {
                    $return .= '<td>'
                        . (
                            $item['editor']
                            ? '<button onclick="javascript:event.stopPropagation'
                                . '();location.href=\'' . $link . '/edit\'">Edit'
                                . (
                                    $item['hasDraft']
                                    ? ' *'
                                    : ''
    ...
    protected function showFilters($id) {
        $formView = new \npdc\view\Form($id);
        list($form, $active) = $formView->create(
            $this->controller->formController->form,
            true,
            true
        );
        return '<h3>Filters ' . $active . '</h3><div class="content">' . $form
            . '</div>';
    }
        ...
        switch ($this->controller->display) {
            case 'not_allowed':
                if ($this->session->userLevel > NPDC_PUBLIC) {
                    ...
                    $this->mid = 'You have insufficient privileges to access '
                        . 'this page';
                } else {
                    $this->title = 'Please login';
                    $this->mid = 'Please login<script type="text/javascript" '
                        . 'language="javascript">$.ready(openOverlay(\''
                        . BASE_URL . '/login?notice=login\'));</script>';
                }
                break;
            case 'under_review':
                $this->title = 'Editing currently not possible';
                $this->mid = 'There is a version of this page submitted for '
                    . 'review, editting is not possible';
                break;
            case 'screen_start':
                header(
                    'Location:'
    ...
    private function loadEditMenu($pages) {
        ...
        foreach ($pages as $url=>$page) {
            $this->left .= '<li><a href="' . $base_url . $url . '"'
                ...
                . (
                    $isNew
                    ? ' onclick="npdc.alert(\'This menu is not available until '
                        . 'you saved the first page\');return false;"'
                    : ''
                )
                . '>'.$page.'</a></li>';
        ...
        $this->left .= '</ul><div class="line"></div>'
            .(
                $isNew 
                ? 'This menu will be activated as soon as you have saved general'
                : ''
            );
    }
}